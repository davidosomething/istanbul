 ...
 * @param array structuredText
 */
function annotateLines(fileCoverage, structuredText) {
    // lineStats is an object with keys string line number
    var lineStats = fileCoverage.l;
    if (!lineStats) { return; }

    var structs = structuredText.length;
    var lineNumbers = Object.keys(lineStats);
    for (var index = 0; index < structs; index++) {
    // Object.keys(lineStats).forEach(function (lineNumber) {
        var lineNumber = lineNumbers[index];
        var count = lineStats[lineNumber];
        var lineData = structuredText[index];
        structuredText[index].covered = count > 0 ? 'yes' : 'no';
    //});
    }
...
function annotateStatements(fileCoverage, structuredText) {
    ...
    Object.keys(statementStats).forEach(function (stName) {
        ...
        var structIndex = null;
        for (var i = 0; i < structs; i++) {
          if (structuredText[i].line === startLine) {
            structIndex = i;
            break;
          }
        }
        ...
        if (type === 'no') {
            if (endLine !== startLine) {
                endLine = startLine;
                endCol = struct.text.originalLength();
            }
            text = struct.text;
            text.wrap(startCol,
                openSpan,
...
function annotateFunctions(fileCoverage, structuredText) {
    ...
    Object.keys(fnStats).forEach(function (fName) {
        ...
        var struct = structuredText[structIndex];
        if (type === 'no') {
            if (endLine !== startLine) {
                endLine = startLine;
                endCol = struct[startLine].text.originalLength();
            }
            text = struct[startLine].text;
            text.wrap(startCol,
                openSpan,
                startLine === endLine ? endCol : text.originalLength(),
                closeSpan);
